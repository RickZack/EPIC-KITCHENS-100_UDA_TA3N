...
for frame_aggregation in avgpool trn-m
	...
	for modality in "RGB Audio" "RGB Flow" "Flow Audio" "RGB Flow Audio"
		...
		then
			...
			-b $bS $bS_2 $bS -j 0 -ef 1 -pf 50 -sf 50 --copy_list N N --save_model --rna $rna --rna_weight $rna_weight \
		...
		then
			...
			--align_modalities $align_modalities --result_filename shared.csv --rna_weight $rna_weight --use_target $use_target
		fi
	done
done
