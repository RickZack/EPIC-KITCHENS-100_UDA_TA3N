...
for frame_aggregation in avgpool trn-m
	...
	for modality in "RGB Audio" "RGB Flow" "Flow Audio" "RGB Flow Audio"
		...
		then
			...
			-b $bS $bS_2 $bS -j 0 -ef 1 -pf 50 -sf 50 --copy_list N N --save_model --additional_net "${additional_net}" --rna_weight $rna_weight \
			--align_modalities "${align_modalities}" --add_net_output $add_net_output
		fi
