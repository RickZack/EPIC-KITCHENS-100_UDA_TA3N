...
for frame_aggregation in avgpool trn-m
	do
for modality in "RGB Audio" "RGB Flow" "Flow Audio" "RGB Flow Audio"
	do
	mod="$(echo -e "${modality}" | xargs | tr ' ' _ )"
	#====== select mode ======#
	if ($training) 
	then
		...
		#------ main command ------#
		echo $modality
		python main.py $num_class $modality \
		$train_source_list $train_target_list $val_list $path_data_val $path_data_source $path_data_target --exp_path $exp_path \
		...
		-b $bS $bS_2 $bS -j 4 -ef 1 -pf 50 -sf 50 --copy_list N N --save_model --rna $rna --rna_weight $rna_weight \
		--seqex $seqex --align_modalities $align_modalities
	...
	then
		...
		python test_models.py $num_class $modality \
		$val_list \
		...
		-j 4 --bS 512 --top 1 3 5 --add_fc 1 --fc_dim $fc_dim --baseline_type $baseline_type --frame_aggregation $frame_aggregation --rna $rna --seqex $seqex \
		--align_modalities $align_modalities --use_target $use_target --rna_weight $rna_weight --result_filename shared.csv
	fi
done
done
