...
for frame_aggregation in avgpool trn-m
	...
	for modality in "RGB Audio" "RGB Flow" "Flow Audio" "RGB Flow Audio"
		...
		then
			...
			-b $bS $bS_2 $bS -j 4 -ef 1 -pf 50 -sf 50 --copy_list N N --save_model --additional_net "${additional_net}" \
			--align_modalities "${align_modalities}" --rna_weight $rna_weight --use_target $use_target --add_net_output $add_net_output
		fi
		...
		then
			...
			--align_modalities "${align_modalities}" --result_filename fc512.csv --rna_weight $rna_weight --use_target $use_target --result_filename shared.csv --add_net_output $add_net_output
		fi
	done
done
